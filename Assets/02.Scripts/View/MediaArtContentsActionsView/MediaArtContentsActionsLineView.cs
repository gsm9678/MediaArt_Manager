...
public class MediaArtContentsActionsLineView : MonoBehaviour
{
    ...
    private MediaArtContentsActionLineViewModel _viewModel;
    private bool _updatingFromViewModel;

    public void Bind(MediaArtContentsActionLineViewModel vm)
    {
        if (_viewModel != null)
            Unbind();
        ...
        // UI → VM
        StartButton.onClick.AddListener (() => GameManager.Instance.MediaArtStartAction?.Invoke(lineNum));
